...
export default class Cohesion {
  ...
  public init(preloadUrl?: string): BrowserWindow {
    ...
    this.window.loadURL(preloadUrl ?? "https://notion.so/login", {
  ...
  private makeLinksOpenInBrowser() {
    // Always deny new windows; open external URLs in the system browser instead.
    // (Internal navigation is handled via loadURL, not window.open.)
